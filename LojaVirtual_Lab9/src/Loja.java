...
public class Loja {
    ...
    public String receberPedido(Vendavel item, int quantidade, Usuario usuario)
        ...
        if (item instanceof Livro) {
            ...
            if(livro.getCategoriaDoLivro() == CategoriaDoLivro.DIDATICO
            && quantidade >= 3){
                precoTotal *= 0.8;
            }
        }
    ...
    public void incluirItemNoEstoque(Vendavel item, int quantidade){
        ...
        Integer valorPreExistente = this.quantidadeEmEstoquePorItem.get(item);
        if(valorPreExistente != null){
            this.quantidadeEmEstoquePorItem.put(item, valorPreExistente + quantidade);
        } else {
