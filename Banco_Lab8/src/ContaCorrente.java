...
public class ContaCorrente {
    ...
    public void depositar(float valor) throws ValorDeDepositoInvalidoException{

        if(valor <= 0){
            // Lança uma ValorDeDepositoInvalidoException quando o valor de depósito passado não é positivo.
            // ToDo lançar uma exceção!!!!
            throw new ValorDeDepositoInvalidoException(valor);
        }
        // altera o saldo
    ...
    public void sacar(float valor)
            throws ValorDeSaqueInvalidoException, SaldoInsuficienteParaSaqueException {
        ...
        if (valorAlemDoLimite > 0) {
            throw new SaldoInsuficienteParaSaqueException(valorAlemDoLimite);
        }
     ...
     */
    public void transferir(float valor, ContaCorrente contaDestino) throws ValorDeTransferenciaInvalidoException, SaldoInsuficienteParaTransferenciaException {
        // valida o parâmetro
        if (valor <= 0) {
            // ToDo lançar uma exceção!!!!
            throw new ValorDeTransferenciaInvalidoException(valor);
        }

        // verifica se há fundos na conta
        float valorAlemDoLimite = (this.saldoEmReais + this.LIMITE_CHEQUE_ESPECIAL) - valor;
        if (valorAlemDoLimite < 0) {
            throw new SaldoInsuficienteParaTransferenciaException(-valorAlemDoLimite);  // ToDo lançar uma exceção!!!!
        }
